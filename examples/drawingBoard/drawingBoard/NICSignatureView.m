...
static inline void addVertex(NSUInteger *length, NICSignaturePoint v) {
    ...
    GLvoid *data = glMapBufferOES(GL_ARRAY_BUFFER, GL_WRITE_ONLY_OES); //更新缓存
    memcpy(data + sizeof(NICSignaturePoint) * (*length), &v, sizeof(NICSignaturePoint)); //拷贝内存内容
...
static float clamp(min, max, value) { return fmaxf(min, fminf(max, value)); } //区域取值 防止溢出
...
- (void)commonInit {
    ...
    tempImage = [UIImage imageNamed:@"temp.jpg"];
}
...
//    CGContextDrawImage(context, self.bounds, tempImage.CGImage);
